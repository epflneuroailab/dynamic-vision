...
FIGURE_DIR = os.path.join(FIGURE_CACHE, 'f4f')
...
def generate_motion_map(args):
    ...
    plot_dual(motion_index_masked)
    
    # Save figure
    output_path = os.path.join(FIGURE_DIR, "map.png")
    plt.savefig(output_path, dpi=600, transparent=True)
    plt.close()
    
    print(f"Motion map saved to: {output_path}")
